...
function _nrate_file
    ...
    # WHY: map rating to interval days (SM-2 inspired, simplified)
    ...
    # NOTE: macOS BSD sed requires `sed -i ''` (with space) not `sed -i''`
    # The latter makes BSD sed treat the next argument as the backup extension.
