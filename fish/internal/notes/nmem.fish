...
# description: Meaningful memorization: memorize key facts, then build understanding.
...
function nmem --description "notes: meaningful memorization"
    if test (count $argv) -eq 0
        ...
        echo "Phase 1: List the facts you MUST know cold."
        echo "Phase 2: Build understanding on that foundation."
        return 1
    end
    ...
    if not test -f "$file"
        ...
        echo "" >>"$file"
        echo "<!-- List every fact, definition, formula, rule. Quantity matters here. -->" >>"$file"
        echo "<!-- This is the foundation. You cannot reason without raw material. -->" >>"$file"
        echo "" >>"$file"
        echo "1. " >>"$file"
        echo "2. " >>"$file"
        echo "3. " >>"$file"
        ...
        echo "" >>"$file"
        echo "<!-- Only attempt this AFTER you can recite Phase 1 from memory -->" >>"$file"
        echo "" >>"$file"
        echo "## Phase 3: Apply -- solve a problem using these facts" >>"$file"
        echo "" >>"$file"
        echo "## Self-test: Cover Phase 1 and write all facts from memory" >>"$file"
        echo "" >>"$file"
