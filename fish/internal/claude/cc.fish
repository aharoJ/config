...
function cc --description '[claude]: roles --model-{opus-46,opus-47,sonnet}; opus=plan-first, sonnet=immediate-bypass'
    ...
    switch $role
        ...
        case opus-47
            claude --model 'claude-opus-4-7[1m]' --effort xhigh --permission-mode plan --allow-dangerously-skip-permissions $passthrough
        case sonnet
            set -lx CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING 1
            set -lx MAX_THINKING_TOKENS 4000
            claude --model claude-sonnet-4-6 --effort low --dangerously-skip-permissions $passthrough
        case '*'
