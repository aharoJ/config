function openrouter --description "Qwen Code via OpenRouter (requires exactly one --model-{name} flag)"
    ...
    if test $flag_count -eq 0
        ...
        echo "" >&2
        echo "Usage:" >&2
        echo "  openrouter --model-qwen-coder-plus    # qwen/qwen3-coder-plus    (paid, code-tuned)" >&2
        echo "  openrouter --model-qwen-3-6-plus      # qwen/qwen3.6-plus        (paid, newest gen)" >&2
    ...
    for arg in $argv
        switch $arg
            case --model-qwen-coder-plus --model-qwen-3-6-plus --model-qwen-max --model-glm
