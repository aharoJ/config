...
# description: Restart yabai and apply a layout profile.
# usage:
#   yr                  → restart with stack profile (default)
#   yr -P bsp           → restart with BSP profile
#   yr -P float         → restart with float profile
#   yr --status         → show current yabai state (no restart)
# date: 2026-02-07

function yr --description "yabai: restart + apply profile"
    ...
    set_color yellow
    echo ""
    echo "✅ yabai restarted (profile: $profile)"
