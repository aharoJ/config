...
local function first_glob(pat)
    local g = fn.glob(pat); if g == "" then return nil end; return vim.split(g, "\n", { trimempty = true })[1]
end
local function all_glob(pat)
    local g = fn.glob(pat); if g == "" then return {} end; return vim.split(g, "\n", { trimempty = true })
end
local function os_config_dir()
    if fn.has("mac") == 1 then return "config_mac" end; if fn.has("unix") == 1 then return "config_linux" end; return
    "config_win"
end
...
local function build_cmd(jdtls_home, workspace_dir)
    ...
    if lombok then table.insert(cmd, 8, "-javaagent:" .. lombok) end -- before -jar is safe
    return cmd
end
